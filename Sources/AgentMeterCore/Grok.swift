...
public enum Grok {
    ...
    public static func parse(_ body: Any) -> ProviderSnapshot {
        ...
        guard let config = JSON.child(record, "config") ?? JSON.object(body) else {
            return .error(.grok, "bad response")
        }
        let used = JSON.number(config["creditUsagePercent"]) ?? 0
        let period = JSON.child(config, "currentPeriod", "current_period") ?? [:]
        let periodType = JSON.string(period["type"]) ?? ""
        let label: String
        if periodType.contains("MONTHLY") {
            label = "30d"
        } else if periodType.contains("WEEKLY") || periodType.isEmpty {
            label = "7d"
        } else {
    ...
    private static func readCredentials() -> Cred? {
        guard let file = JSON.parseFile(Paths.home(".grok", "auth.json")) else {
            return nil
        }
        var candidates: [Cred] = []
        for obj in JSON.walkObjects(file) {
            guard let key = JSON.string(obj["key"]) else { continue }
            candidates.append(
                Cred(
                    key: key,
        ...
        return candidates.max(by: { $0.createTime < $1.createTime })
    }
}
